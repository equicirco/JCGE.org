...
Return a RunSpec for the monopoly CGE model port (Chapter 10.4).
"""
function model(; sam_path::Union{Nothing,AbstractString} = nothing,
    ...
    sigma = Dict(i => 3.0 for i in commodities)
    ...
    D0 = Dict(i => (Q0[i] - (1.0 + tau_m[i]) * M0[i]) / (1.0 / eta[i]) for i in commodities)
    RT0 = Dict(i => (1.0 - eta[i]) / eta[i] * D0[i] for i in commodities)

    F0 = Dict{Tuple{Symbol,Symbol},Float64}()
    for h in factors_sym, j in activities
        denom = sum(sam[k, j] for k in factors_sym)
        F0[(h, j)] = sam[h, j] - sam[h, j] / denom * RT0[j]
    end

    FF = Dict(h => sum(F0[(h, j)] for j in activities) for h in factors_sym)
    ...
    deltam = Dict(i => (1.0 + tau_m[i]) * M0[i] ^ (1.0 - eta[i]) /
        ((1.0 + tau_m[i]) * M0[i] ^ (1.0 - eta[i]) + (1.0 / eta[i]) * D0[i] ^ (1.0 - eta[i])) for i in commodities)
    deltad = Dict(i => (1.0 / eta[i]) * D0[i] ^ (1.0 - eta[i]) /
        ((1.0 + tau_m[i]) * M0[i] ^ (1.0 - eta[i]) + (1.0 / eta[i]) * D0[i] ^ (1.0 - eta[i])) for i in commodities)
    ...
    ssp = Sp0 / (sum(values(FF)) + sum(values(RT0)))
    ssg = Sg0 / (Td0 + sum(values(Tz0)) + sum(values(Tm0)))
    tau_d = Td0 / (sum(values(FF)) + sum(values(RT0)))
    ...
    arm_params = (
        ...
        eta = eta,
        tau_m = tau_m,
        pd_scale = Dict(i => 1.0 / eta[i] for i in commodities),
    )
    ...
    rent_block = JCGEBlocks.monopoly_rent(:rent, commodities, (eta = eta,))
    ...
    for j in commodities
        ...
        start_vals[JCGEBlocks.global_var(:RT, j)] = RT0[j]
    end
    ...
    push!(section_blocks[:markets], rent_block, market_block)
