...
function _load_data()
    ...
    x0 = Dict(i => xxd0[i] + (i in traded ? m0[i] : 0.0) for i in sectors)
    pm0 = pd0
    pe0 = pd0
...
function model()
    ...
    trade_block = JCGEBlocks.trade_price_link(:trade_prices, sectors, (traded=data.traded, te=data.te))
    absorption_block = JCGEBlocks.absorption_sales(:absorption, sectors, (traded=data.traded,))
    activity_price_block = JCGEBlocks.activity_price_io(:activity_price, sectors, sectors, (io=data.io, itax=data.itax))
    capital_price_block = JCGEBlocks.capital_price_composition(:capital_price, sectors, sectors, (imat=data.imat,))
    production_block = JCGEBlocks.production_multilabor_cd(:production, sectors, labor; params=(ad=data.ad, alphl=data.alphl, wdist=data.wdist))
    labor_block = JCGEBlocks.labor_market_clearing(:labor_market, labor, sectors; params=(;))
    cet_block = JCGEBlocks.cet_xxd_e(:cet, sectors, (traded=data.traded, at=data.at, gamma=data.gamma, rhot=data.rhot))
    export_block = JCGEBlocks.export_demand(:export, sectors, (traded=data.traded, eta=data.eta, e0=data.e0, pwe0=data.pwe0))
    armington_block = JCGEBlocks.armington_m_xxd(:armington, sectors, (traded=data.traded, delta=data.delta, ac=data.ac, rhoc=data.rhoc))
    nontraded_block = JCGEBlocks.nontraded_supply(:nontraded, sectors, (nontraded=data.nontraded,))
    inventory_block = JCGEBlocks.inventory_demand(:inventory, sectors, (dstr=data.dstr,))
    household_block = JCGEBlocks.household_share_demand(:household, sectors, (cles=data.cles,))
    government_demand_block = JCGEBlocks.government_share_demand(:government_demand, sectors, (gles=data.gles,))
    government_finance_block = JCGEBlocks.government_finance(:government_finance, sectors, (traded=data.traded, itax=data.itax, te=data.te))
    gdp_block = JCGEBlocks.gdp_income(:gdp, sectors, (;))
    savings_block = JCGEBlocks.savings_investment(:savings, sectors, sectors, (depr=data.depr, kio=data.kio, imat=data.imat))
    market_block = JCGEBlocks.final_demand_clearing(:market, sectors, (;))
    objective_block = JCGEBlocks.consumption_objective(:objective, sectors, (alpha=data.cles,))
