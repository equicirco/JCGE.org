...
function model()
    ...
    trade_block = JCGEBlocks.trade_price_link(
        :trade_price,
        sectors,
        (traded = traded, te = te_vals, include_pr = true, pedef_mode = :pwe, pwe = pwe_vals, pwm = pwm_vals, mcp = true),
    )
    absorption_block = JCGEBlocks.absorption_sales(:absorption, sectors, (traded = traded, mcp = true))
    activity_price_block = JCGEBlocks.activity_price_io(
        :activity_price,
        sectors,
        sectors,
        (io = io_vals, itax = itax_vals, mcp = true),
    )
    capital_price_block = JCGEBlocks.capital_price_composition(:capital_price, sectors, sectors, (imat = imat_vals, mcp = true))
    production_block = JCGEBlocks.production_multilabor_cd(
        :production,
        sectors,
        labor;
        params = (ad = ad_vals, wdist = wdist_vals, alphl = alphl_vals, mcp = true),
    )
    labor_block = JCGEBlocks.labor_market_clearing(:labor_market, labor, sectors; params = (mcp = true,))
    cet_block = JCGEBlocks.cet_xxd_e(:cet, sectors, (at = at_vals, gamma = gamma_vals, rhot = rhot_vals, traded = traded, mcp = true))
    armington_block = JCGEBlocks.armington_m_xxd(:armington, sectors, (ac = ac_vals, delta = delta_vals, rhoc = rhoc_vals, traded = traded, mcp = true))
    nontraded_block = JCGEBlocks.nontraded_supply(:nontraded, sectors, (nontraded = nontraded, mcp = true))
    inventory_block = JCGEBlocks.inventory_demand(:inventory, sectors, (dstr = dstr_vals, mcp = true))
    household_demand_block = JCGEBlocks.household_share_demand_hh(
        :household_demand,
        households,
        sectors;
        params = (cles = cles_vals, htax = htax_vals, mcp = true),
    )
    household_income_block = JCGEBlocks.household_income_labor_capital(
        ...
        sectors,
        labor;
        params = (labor_household = labor_hh, capital_household = capital_hh, mcp = true),
    )
    household_tax_block = JCGEBlocks.household_tax_revenue(:household_tax, households; params = (htax = htax_vals, mcp = true))
    household_sum_block = JCGEBlocks.household_income_sum(:household_sum, households; params = (mcp = true,))
    government_demand_block = JCGEBlocks.government_share_demand(:government_demand, sectors, (gles = gles_vals, mcp = true))
    government_revenue_block = JCGEBlocks.government_revenue(
        :government_revenue,
        sectors;
        params = (itax = itax_vals, te = te_vals, traded = traded, pwe = pwe_vals, pwm = pwm_vals, mcp = true),
    )
    premium_block = JCGEBlocks.import_premium_income(:premium_income, sectors, (traded = traded, pwm = pwm_vals, mcp = true))
    savings_block = JCGEBlocks.savings_investment(
        :savings,
        sectors,
        sectors,
        (depr = depr_vals, kio = kio_vals, imat = imat_vals, use_invest = true, mcp = true),
    )
    price_index_block = JCGEBlocks.price_index(:price_index, sectors, (pwts = pwts_vals, mcp = true))
    bop_block = JCGEBlocks.external_balance_remit(:bop, sectors, (traded = traded, pwe = pwe_vals, pwm = pwm_vals, mcp = true))
    market_block = JCGEBlocks.final_demand_clearing(:market, sectors, (mcp = true,))
    objective_block = JCGEBlocks.consumption_objective(:objective, sectors, (alpha = alpha_vals, mcp = true))
