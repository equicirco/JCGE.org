...
Return a RunSpec for the scale-economy CGE model port (Chapter 10.6).
"""
function model(; sam_path::Union{Nothing,AbstractString} = nothing,
    ...
    cap = :CAP in factors_sym ? :CAP : factors_sym[1]
    nu = Dict(i => 0.1 for i in commodities)
    ...
    for h in factors_sym, j in activities
        if h == cap
            F0[(h, j)] = (1.0 - nu[j]) * sam[h, j]
        else
            F0[(h, j)] = sam[h, j]
        end
    end
    FC = Dict(j => nu[j] * sam[cap, j] for j in activities)
    ...
    Z0 = Dict(j => Y0[j] + sum(X0[(i, j)] for i in commodities) + FC[j] for j in activities)
    ...
    ssp = Sp0 / (sum(values(FF)) + sum(values(FC)))
    ssg = Sg0 / (Td0 + sum(values(Tz0)) + sum(values(Tm0)))
    tau_d = Td0 / (sum(values(FF)) + sum(values(FC)))

    prod_params = (b = b, beta = beta, ay = ay, ax = ax, FC = FC)
    ...
    saving_block = JCGEBlocks.private_saving(:private_saving, factors_sym, (ssp = ssp, FF = FF, FC = FC, include_fc = true))

    hh_params = (alpha = alpha, FF = FF, FC = FC, include_fc = true)
